...
const Projects = () => {
  ...
  // Subtle "jelly" hover tilt/parallax animation
  const handleMouseMove = (index, e) => {
    ...
    const rotateMax = 6; // smaller = smoother
    const txMax = 10; // parallax intensity

    card.style.transform = `perspective(800px) rotateX(${-py * rotateMax}deg) rotateY(${px * rotateMax}deg) scale(1.03)`;
    card.style.transition = "transform 120ms linear";

    img.style.transform = `translate3d(${-px * txMax}px, ${-py * txMax}px, 0) scale(1.05)`;
  ...
  const handleMouseLeave = (index) => {
    ...
    card.style.transition = "transform 600ms cubic-bezier(.2,.9,.1,1)";

    img.style.transform = "translate3d(0,0,0) scale(1)";
    img.style.transition = "transform 600ms cubic-bezier(.2,.9,.1,1)";
  };
  ...
  return (
    ...
    >
      <div className="lg:container mx-auto px-6 md:px-8 lg:px-12">
        <h1 className="text-center text-3xl md:text-4xl text-gray-950 dark:text-gray-50 font-bold tracking-wider">
          Featured Projects
        </h1>

        <div className="mt-4 h-0.5 w-32 mx-auto bg-gradient-to-r from-gray-100 via-gray-400 to-gray-100 dark:from-gray-900 dark:via-gray-400 dark:to-gray-900"></div>

        <p className="mt-6 text-lg text-gray-600 dark:text-gray-300 text-center">
          “Projects that showcase my frontend skills and full-stack capabilities.”
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mt-8">
          {[...inDevelopmentProject, ...projects].map((project, index) => (
            <motion.div
              key={index}
              ...
              viewport={{ once: true }}
              className="group bg-gray-200 dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 hover:shadow-lg transition-all duration-300 overflow-hidden h-fit md:h-[29rem] flex flex-col"
            >
              <div className="h-32 md:h-2/5 bg-gradient-to-br from-gray-900 to-gray-800 flex justify-center items-center overflow-hidden">
                <div className="relative w-full h-full flex justify-center items-center">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-6">
                    <a
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 text-gray-300 rounded-lg bg-white/10 backdrop-blur-sm border border-white/10 hover:bg-white/30 hover:border-gray-500 transition-colors duration-300"
                    >
              ...
              <div className="px-6 py-6 md:py-4 flex flex-col">
                <div className="flex flex-row justify-between items-center mb-4">
                  <p className="text-lg font-semibold text-gray-600 dark:text-gray-50">
                    {project.title}
                  </p>
                  <span
                    className={`text-sm font-medium px-3 py-1 rounded-full ${
                      index < inDevelopmentProject.length
                        ? "bg-yellow-500/30 text-yellow-700 dark:text-yellow-300"
                        : "bg-emerald-500/30 text-emerald-700 dark:text-emerald-300"
                    } border border-gray-400/20`}
                  >
                    {index < inDevelopmentProject.length ? "In Development" : "Live"}
                  </span>
                </div>

                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 md:line-clamp-3">
                  {project.description}
                </p>
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {project.tags.map((tag, tagIndex) => (
                    <span
                      key={tagIndex}
                      className="text-xs text-gray-600 dark:text-gray-50 font-medium bg-gray-50 dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-600 py-1 px-2"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex gap-3">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 px-4 py-2 text-gray-300 rounded-lg bg-gray-800 dark:bg-gray-700 hover:bg-gray-900 transition"
                  >
                    <ExternalLink size={18} strokeWidth={2} />
